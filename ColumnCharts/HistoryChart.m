...
//        _columnSpace = TwoPointSpacing(CanvasRightBottomPoint, CanvasLeftBottomPoint) / _column;
        _columnSpace = 2*padding+_histogramWidth*4;
    }
    return _columnSpace;
}
#pragma mark - 设置初始值 -
- (void)customDefaultParams{
    ...
    _line1ChartColor = [UIColor colorWithRed:67 / 255.0 green:155 / 255.0 blue:231 / 255.0 alpha:1];
    _line2ChartColor = [UIColor colorWithRed:47 / 255.0 green:69 / 255.0 blue:84 / 255.0 alpha:1];
    _line3ChartColor = [UIColor colorWithRed:1 / 255.0 green:155 / 255.0 blue:231 / 255.0 alpha:1];
    _line4ChartColor = [UIColor colorWithRed:194 / 255.0 green:53 / 255.0 blue:49 / 255.0 alpha:1];
    _lineChartWidth = 1;
...
- (void)reloadData{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(updateViewWidth:)]) {
        NSInteger totalWidth = self.columnSpace*(self.columnData.count) + _edgeInsets.left + _edgeInsets.right;
        _size.width = totalWidth;
        [_delegate updateViewWidth:totalWidth];
    }
    // 绘制XY轴
    [self drawXYAxis];
    // 绘制辅助线
...
- (void)columnChart1
{
    ...
    for (int i = 0; i < _valueDataAry1.count; i++) {
        ...
        histogramLayer.fillColor = _line1ChartColor.CGColor;
...
- (void)columnChart2
{
    ...
    for (int i = 0; i < [_valueDataAry2 count]; i++) {
        ...
        CGPoint point = POINT(CanvasLeftBottomPoint.x + (padding+_histogramWidth+i  * self.columnSpace), CanvasLeftBottomPoint.y + 0.5);
        ECAnimationLayer *histogramLayer = [self histogramWith:point width:_histogramWidth height:TwoPointSpacing(CanvasLeftTopPoint, CanvasLeftBottomPoint)];
        histogramLayer.anchorPoint = CGPointMake(0, 0);
        histogramLayer.zPosition = 999999;
        histogramLayer.fillColor = _line2ChartColor.CGColor;
        [self.layer addSublayer:histogramLayer];
        [self.chartLayerAry addObject:histogramLayer];
        CGFloat curheight = [curValue floatValue] * scale;
        histogramLayer.selfRect = CGRectMake(point.x, point.y, _histogramWidth, curheight);
...
- (void)columnChart3
{
    CGFloat scale = self.rowSpace / space_value;
    NSLog(@"左边距 = CanvasLeftBottomPoint.x = %f %f",CanvasLeftBottomPoint.x,CanvasLeftBottomPoint.y);
    for (int i = 0; i < [_valueDataAry3 count]; i++) {
        NSString *curValue = _valueDataAry3[i];
        CGPoint point = POINT(CanvasLeftBottomPoint.x + (padding+_histogramWidth*2+i  * self.columnSpace), CanvasLeftBottomPoint.y + 0.5);
        ECAnimationLayer *histogramLayer = [self histogramWith:point width:_histogramWidth height:TwoPointSpacing(CanvasLeftTopPoint, CanvasLeftBottomPoint)];
        histogramLayer.anchorPoint = CGPointMake(0, 0);
        histogramLayer.zPosition = 999999;
        histogramLayer.fillColor = _line3ChartColor.CGColor;
        [self.layer addSublayer:histogramLayer];
        [self.chartLayerAry addObject:histogramLayer];
        CGFloat curheight = [curValue floatValue] * scale;
        histogramLayer.selfRect = CGRectMake(point.x, point.y, _histogramWidth, curheight);
...
- (void)columnChart4
{
    CGFloat scale = self.rowSpace / space_value;
    NSLog(@"左边距 = CanvasLeftBottomPoint.x = %f %f",CanvasLeftBottomPoint.x,CanvasLeftBottomPoint.y);
    for (int i = 0; i < [_valueDataAry4 count]; i++) {
        NSString *curValue = _valueDataAry4[i];
        CGPoint point = POINT(CanvasLeftBottomPoint.x + (padding+_histogramWidth*3+i  * self.columnSpace), CanvasLeftBottomPoint.y + 0.5);
        ECAnimationLayer *histogramLayer = [self histogramWith:point width:_histogramWidth height:TwoPointSpacing(CanvasLeftTopPoint, CanvasLeftBottomPoint)];
        histogramLayer.anchorPoint = CGPointMake(0, 0);
        histogramLayer.zPosition = 999999;
        histogramLayer.fillColor = _line4ChartColor.CGColor;
        [self.layer addSublayer:histogramLayer];
        [self.chartLayerAry addObject:histogramLayer];
        CGFloat curheight = [curValue floatValue] * scale;
        histogramLayer.selfRect = CGRectMake(point.x, point.y, _histogramWidth, curheight);
...
- (CGFloat)spaceValue{
    ...
    CGFloat max1 = [[_valueDataAry1 valueForKeyPath:@"@max.floatValue"] floatValue];
    CGFloat max2 = [[_valueDataAry2 valueForKeyPath:@"@max.floatValue"] floatValue];
    CGFloat max3 = [[_valueDataAry3 valueForKeyPath:@"@max.floatValue"] floatValue];
    CGFloat max4 = [[_valueDataAry4 valueForKeyPath:@"@max.floatValue"] floatValue];
    CGFloat min1 = [[_valueDataAry1 valueForKeyPath:@"@min.floatValue"] floatValue];
    CGFloat min2 = [[_valueDataAry2 valueForKeyPath:@"@min.floatValue"] floatValue];
    CGFloat min3 = [[_valueDataAry3 valueForKeyPath:@"@min.floatValue"] floatValue];
    CGFloat min4 = [[_valueDataAry4 valueForKeyPath:@"@min.floatValue"] floatValue];
    
    minValue = MIN(MIN(min1, min2), MIN(min3, min4));
    maxValue = MAX(MAX(max1, max2), MAX(max3, max4));
